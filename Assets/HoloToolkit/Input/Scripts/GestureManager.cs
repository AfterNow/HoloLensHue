...
namespace HoloToolkit.Unity
{
    ...
    /// 
    /// Using Manipulation or Navigation requires subscribing the the ManipulationStarted or NavigationStarted events and then querying
    /// information about the manipulation or navigation gesture via ManipulationOffset and ManipulationHandPosition or NavigationOffset and NavigationHandPosition.
    /// 
    ...
    public partial class GestureManager : Singleton<GestureManager>
    {
        ...
        /// <summary>
        /// Occurs when a navigation gesture has started.
        /// </summary>
        /// <param name="sourceKind">The Interaction Source Kind that started the event.</param>
        public delegate void NavigationStartedDelegate(InteractionSourceKind sourceKind);
        public event ManipulationStartedDelegate OnNavigationStarted;

        /// <summary>
        /// Occurs when a navigation gesture ended as a result of user input.
        /// </summary>
        /// <param name="sourceKind">The Interaction Source Kind that completed the event.</param>
        public delegate void NavigationCompletedDelegate(InteractionSourceKind sourceKind);
        public event ManipulationCompletedDelegate OnNavigationCompleted;

        /// <summary>
        /// Occurs when a navigation gesture ended as a result of some other condition.
        /// (e.g. the hand being used for the gesture is no longer visible).
        /// </summary>
        /// <param name="sourceKind">The Interaction Source Kind that cancelled the event.</param>
        public delegate void NavigationCanceledDelegate(InteractionSourceKind sourceKind);
        public event ManipulationCanceledDelegate OnNavigationCanceled;
        ...
        /// Valid if a manipulation gesture is in progress.
        /// </summary>
        public Vector3 NavigationOffset { get; private set; }

        /// <summary>
        /// The world space position of navigation source being used for the current navigation gesture.
        /// Valid if a navigation gesture is in progress.
        /// </summary>
        ...
        /// <summary> We use a separate manipulation and navigation recognizer here because the tap gesture recognizer cancels
        ...
        private void Awake()
        {
            ...
            // Create new GestureRecognizer.
            // Sign up for navigation events.
            ...
            // Start looking for navigation gestures
        ...
        /// <param name="inProgress">Is this manipulation in progress?</param>
        /// <param name="offset">The Offset of our manipulation to calulate delta positions.</param>
        private void NavigationRecognizer_NavigationStartedEvent(InteractionSourceKind source, Vector3 cumulativeDelta, Ray headRay)
        {
            // Don't start another navigation gesture if one is already underway
            if (!NavigationInProgress)
            {
                OnNavigation(inProgress: true, offset: cumulativeDelta);
                if (OnNavigationStarted != null)
                {
                    OnNavigationStarted(source);
                }
            }
        }
        ...
        {
            OnNavigation(false, cumulativeDelta);
            if (OnNavigationCanceled != null)
            {
                OnNavigationCanceled(source);
            }
        }

        private void OnNavigation(bool inProgress, Vector3 offset)
        {
            NavigationInProgress = inProgress;

            // If we're doing a navigation set the offset, else reset it to zero.
            NavigationOffset = inProgress ? offset: Vector3.zero;
        }
